...
abstract class TSFlaggedActiveRecord extends \app\components\db\SingletonActiveRecord
{
    ...
    public function flagsFromText($flags)
    {
        ...
        if (is_string($flags)) {
			...
			die('только строку передовать сюда'); // Этого же класса нету. как бы тут не должно работать 8(
            $flags = CPropertyValue::ensureArray($flags);
        }
		
        $result = 0;
        foreach ($flags as $value) {
            if(!isset($flagsValues[$value])){
                return;
            }
            ...
            if ($flagValue) {
                ...
                return; //throw new Exception(Yii::t('ts.ext', 'Flag {value} is not known string flag value', array('{value}' => $value)));
            }
        }
